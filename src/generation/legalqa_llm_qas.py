"""
legal_qa_neural_pathway_analyzer.py
=====================================
Prompt-engineering evaluation harness for knowledge-based Q&A over an
Australian Law ontology (case law, legislation, parties, jurisdictions,
...
Yes/No answers must include an inline legal explanation; bare Yes/No is rejected.
Fabricated citations, case names, or statutory provisions are explicitly prohibited
in every prompt variant.
...
class PromptBuilder:
    ...
    Each prompt is composed from four reusable blocks:
        ...
        _ROLE_BLOCK          — establishes persona and legal domain expertise
        _GOAL_BLOCK          — states the answering contract, including the
                               anti-hallucination rule for legal citations
        _OUTPUT_CONSTRAINTS  — enforces structural rules (ANSWER: tag, Yes/No
                               policy, citation accuracy requirement)
    ...
    -------------------------
    Legal Q&A is high-stakes: a fabricated citation presented as fact causes
    direct harm. The prohibition appears in _GOAL_BLOCK, _OUTPUT_CONSTRAINTS,
    _SPARQL_FRAMING, and the ANSWER step of _COT_BLOCK to ensure it survives
    regardless of which blocks are combined.
    """

    # ── Shared building blocks ────────────────────────────────────────────── #

    _ROLE_BLOCK = (
        "You are a legal research assistant specialising in Australian Law, "
        "with expertise in case law, legislation, and legal principles across "
        "all Australian jurisdictions including federal, state, and territory courts."
    )

    _GOAL_BLOCK = (
        "Goal: Provide a factually accurate, legally precise, and well-reasoned answer "
        "to the query based strictly on established Australian law, case law, and legislation. "
        ...
        "concise legal explanation — never output a bare 'Yes' or 'No' without elaboration. "
        "Do NOT fabricate case names, citations, legislation, or legal principles. "
        "If the answer is genuinely unknown or outside your knowledge, state so explicitly "
        "and explain why the information cannot be confirmed.\n"
    ...
    _OUTPUT_CONSTRAINTS = (
        ...
        "- If your answer starts with Yes or No, append a colon and a clear legal explanation "
        "on the same line, e.g. 'ANSWER: Yes — <explanation>' or 'ANSWER: No — <explanation>'.\n"
        "- Use precise legal language; cite specific cases, statutes, or sections only when "
        "you are certain of their accuracy — do not guess or approximate citations.\n"
        "- If a case name, citation, or statutory provision cannot be confirmed with certainty, "
        ...
        "- Never fabricate legal authorities, case outcomes, judicial reasoning, or legislation."
    )

    _COT_BLOCK = (
        "Reasoning requirement:\n"
        "Before giving your final answer, work through the problem using this structure:\n"
        "  THINK: Identify the core legal question, relevant jurisdiction, and applicable "
        "area of Australian law.\n"
        "  PLAN: List the legal sources, cases, or statutes needed to reach the answer.\n"
        "  REASON: Apply those legal sources step-by-step, verifying each fact or "
        "citation before relying on it.\n"
        "  ANSWER: Begin with the tag ANSWER: then state your final legal response. "
        "If the answer starts with Yes or No, follow it immediately with a legal "
        "explanation on the same line. Never fabricate citations or case outcomes "
        "in the ANSWER.\n"
        "Keep THINK/PLAN/REASON concise. Only the ANSWER section is shown to the end user."
    )

    _SPARQL_FRAMING = (
        "The following is a SPARQL query submitted to an Australian legal system"
        "containing case law, legislation, parties, jurisdictions, and judicial decisions. "
        "Interpret the query intent and answer as if you had executed it against a "
        "knowledge source. Return only the resolved answer in natural language. "
        "Do not fabricate case names, citations, or legal outcomes that cannot be "
        "confirmed from established Australian legal sources."
    )
...
class ModelManager:
    ...
    CONFIGS: Dict[str, ModelConfig] = {
        ...
        "mistral": ModelConfig(
            ...
            max_sequence_length=1800, generation_max_new_tokens=1024,
            requires_auth=False, prefer_4bit=True, cpu_offload_threshold=0.75,
...
class InferenceRunner:
    ...
    def generation_config(self) -> GenerationConfig:
        return GenerationConfig(
            ...
            # loops without distorting fluent legal prose.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Legal QA Neural Pathway Analyzer — prompt-engineering harness for Australian Law Q&A",
...
Examples
────────
  python legal_qa_neural_pathway_analyzer.py --input queries.xlsx --model mistral    --mode nlq
  python legal_qa_neural_pathway_analyzer.py --input queries.xlsx --model llama3     --mode nlq_cot
  python legal_qa_neural_pathway_analyzer.py --input queries.xlsx --model qwen       --mode sparql --max_queries 50
  python legal_qa_neural_pathway_analyzer.py --input queries.xlsx --model qwen-coder --mode sparql_cot
  python legal_qa_neural_pathway_analyzer.py --input queries.xlsx --model gemma      --mode nlq --no-resume
        """,
    )
    parser.add_argument(
        "--input", required=True,
    ...
    print("=" * 72)
    print("LEGAL QA NEURAL PATHWAY ANALYZER")
    print("=" * 72)
